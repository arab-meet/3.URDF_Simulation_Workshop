...
* **X-axis** : Points **forward** (in the robot's direction of movement).
* **Y-axis** : Points **to the left** of the robot.
* **Z-axis** : Points  **upward** .

This follows the  **right-hand rule** , where the Z-axis points upward when you curl the fingers of your right hand from the X-axis to the Y-axis. For short-range geographic locations, the **East-North-Up (ENU)** convention is used:

* **X-axis** : East
* **Y-axis** : North
* **Z-axis** : Up

For cameras, an alternative "_optical" frame is used where:

* **Z-axis** : Points **forward** (in the camera's direction of view).
* **X-axis** : Points  **right** .
* **Y-axis** : Points  **down** .

### **Rotation Representation**

In ROS, rotations are represented primarily using  **quaternions** , which avoid singularities and provide a compact representation. If quaternions are not used, the alternatives are:

1. **Rotation matrix** (avoids singularities).
2. **Fixed-axis roll, pitch, and yaw** about the X, Y, and Z axes, respectively (used for angular velocities).
3. **Euler angles** (yaw, pitch, roll about Z, Y, X axes), though these are generally discouraged due to ambiguity and multiple valid conventions.
...
For detailed information , visit the official  **[REP 103 document](https://www.ros.org/reps/rep-0103.html)** .
...
## **Step 4: Setting the Axis of Rotation for Yaw Movements**
